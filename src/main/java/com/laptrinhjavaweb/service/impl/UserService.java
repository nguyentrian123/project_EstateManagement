...
public class UserService implements IUserService {
	...
	public List<StaffResponseDTO> findStaffAssginsBuilding(Long buildingId) {
		...
		List<UserEntity> staffEntities = userRepository.findByStatusAndRoles_Code(1, "STAFF");

        for(UserEntity item : staffEntities)
        {
            StaffResponseDTO staffResponseDTO = new StaffResponseDTO();
            staffResponseDTO.setId(item.getId());
            staffResponseDTO.setFullName(item.getFullName());
            boolean isExist = userRepository.existsByIdAndBuildings_Id(item.getId(),buildingId);
            if(isExist){
	...
	public List<StaffResponseDTO> findStaffAssginsCustomer(Long customerId) {
		List<UserEntity> staffsAssigned = userRepository.findByStatusAndRoles_CodeAndCustomers_Id(1, SystemConstant.STAFF, customerId);
		List<UserEntity> staffEntitys = userRepository.findByStatusAndRoles_Code(1, "STAFF");
		List<StaffResponseDTO> staffResponseDTOs = new ArrayList<>();
